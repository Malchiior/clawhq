...
// Token lifespans — users stay logged in until they explicitly log out
const ACCESS_TOKEN_LIFETIME = '365d' // 1 year
const REFRESH_TOKEN_LIFETIME = '365d' // 1 year
...
export function generateTokenPair(userId: string): TokenPair {
  ...
  const refreshExpiresAt = new Date(now.getTime() + 365 * 24 * 60 * 60 * 1000) // 1 year
